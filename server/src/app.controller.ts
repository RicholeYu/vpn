...
export class AppController {
  ...
  @Get('')
  handle(@Headers('remote_addr') remoteAddress: string, @Res() response: Response) {
    if (this.cache[remoteAddress]) {
      return response.redirect(301, `http://vpn.richole.cn/go/has/${remoteAddress}`)
    }

    if (remoteAddress && this.ipReg.test(remoteAddress)) {
      return response.redirect(301, `http://vpn.richole.cn/go/to/${remoteAddress}`)
    }

    return response.redirect(301,  `http://vpn.richole.cn/go/error/${remoteAddress}`)
  }

  @Get('add/:remoteAddress')
  add(@Param('remoteAddress') remoteAddress: string, @Res() response: Response) {
    if (remoteAddress && this.ipReg.test(remoteAddress)) {
      ...
      return response.redirect(301, `http://vpn.richole.cn/go/set/${remoteAddress}`)
    }
